...
final class DiaryRepository extends Repository
{
    ...
    public function getEntryForViewer(int $logId, int $viewerId): ?array
    {
        $query = $this->connection()->prepare(
            "SELECT
                de.id AS log_id,
             ...
             LEFT JOIN genres g ON g.id = fg.genre_id
             WHERE de.id = :log_id
               AND (de.user_id = :viewer_id OR de.is_public = TRUE)
             GROUP BY de.id, u.id, f.id, r.id
             LIMIT 1"
        );
        $query->bindValue(':log_id', $logId, PDO::PARAM_INT);
        $query->bindValue(':viewer_id', $viewerId, PDO::PARAM_INT);
        $query->execute();
